...
// Changes:
// 1) Clearer worker logging (start/end + send/recv steps).
// 2) Added a typed msg_id in the payload.
...
fn main() {
    ...
    // - RabbitMQMImpl for indirect communication via RabbitMQ (backend)
...
/// Minimal point-to-point protocol:
/// - worker 0 sends a message with msg_id
...
fn run_worker(mw: Middleware<StringMessage>) {
    ...
    if id == 0 {
        ...
        // Correctness check: ensure the ACK corresponds to our msg_id.
        let expected = format!("msg_id={}", msg_id);
        if reply.0.contains(&expected) {
            info!("worker {}: ACK matches {}", id, expected);
        } else {
            info!("worker {}: ACK does NOT match {} (got: {})", id, expected, reply.0);
        }
    } else if id == 1 {
